...
class RpcppeController extends Controller
{
    ...
    public function bulkDestroy(Request $request)
    {
        ...
        $request->validate([
            ...
            'bulk_action_type' => 'required|in:permanent,dispose'
        ]);
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('rpcppe.index')->with('success', count($request->ids) . ' items moved to Disposal.');
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error("Bulk Disposal Error: " . $e->getMessage());
            return redirect()->back()->with('error', 'Database Error: ' . $e->getMessage());
        }
    }
